...
const Translator = ({ text }) => {
  ...
  const handleTranslate = async () => {
    ...
    setLoading(true);
    try {
      const res = await translateTextWithAI(inputText, targetLang);
    ...
    <div className="p-4 lg:p-8 max-w-6xl mx-auto space-y-6">
      ...
      <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm flex flex-wrap items-center justify-between gap-4">
        ...
        <button
          ...
          className="px-5 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white font-bold text-xs shadow-md transition-colors flex items-center gap-2"
        >
      ...
      <div className="bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm flex items-center justify-between gap-4">
        ...
        <button
          onClick={handleSwap}
          className="p-2.5 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 text-slate-700 dark:text-slate-200 mt-4 transition-colors"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-white dark:bg-slate-900 p-5 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm space-y-3">
          ...
          <textarea
            ...
            className="w-full min-h-[250px] p-4 rounded-xl bg-slate-50 dark:bg-slate-800/40 text-slate-900 dark:text-slate-100 text-sm focus:outline-none resize-none border border-slate-200/60 dark:border-slate-700/60"
          />
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 p-5 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm space-y-3 flex flex-col justify-between">
          <div className="space-y-3">
            ...
            <div className="min-h-[250px] p-4 rounded-xl bg-blue-50/40 dark:bg-blue-950/20 border border-blue-100 dark:border-blue-900/40 text-slate-900 dark:text-slate-100 text-sm leading-relaxed whitespace-pre-wrap">
              {translatedText || <span className="italic text-slate-400">Translated output will appear here...</span>}
            </div>
          </div>
          ...
          <div className="pt-2 flex gap-2">
            ...
            <button
              onClick={handlePDF}
              disabled={!translatedText}
              className="px-3 py-2 rounded-xl bg-teal-600 hover:bg-teal-700 text-white font-bold text-xs transition-colors flex items-center gap-1 shadow-sm"
            >
            ...
            <button
              onClick={handleDOCX}
              disabled={!translatedText}
              className="px-3 py-2 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-bold text-xs transition-colors flex items-center gap-1 shadow-sm"
            >
              <Download className="w-3.5 h-3.5" />
              <span>DOCX</span>
            </button>
